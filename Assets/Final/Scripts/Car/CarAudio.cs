...
public class CarAudio : MonoBehaviour
{
    ...
    public float pitchMultiplier = .8f;                                         // Used for altering the pitch of audio clips
    ...
    private void Update()
    {
        ...
        if (startedSound)
        {
            ...
            float pitch = Mathf.LerpUnclamped(lowPitchMin, lowPitchMax, carController.Revs);
    ...
    private void StartSound()
    {
        ...
        highAccel = SetUpEngineAudioSource(highAccelClip, audioMixerGroup);

        // if we have four channel audio setup the four audio sources
        if (engineSoundStyle == EngineAudioOptions.FourChannel)
        {
            lowAccel = SetUpEngineAudioSource(lowAccelClip, audioMixerGroup);
            lowDecel = SetUpEngineAudioSource(lowDecelClip, audioMixerGroup);
            highDecel = SetUpEngineAudioSource(highDecelClip, audioMixerGroup);
        }
    ...
    private AudioSource SetUpEngineAudioSource(AudioClip clip, AudioMixerGroup audioMixerGroup = null)
    {
